...
function App() {
  ...
  const shellClass = location.pathname === '/' && isLeavingAbout ? 'site-shell home-returning' : 'site-shell';
  return <div className={shellClass}><Header /><ScrollProgress /><LiquidFilter /><RouteVisualTransition />{showAboutTransition && <div className={`page-transition about-transition ${transitionClass}`} aria-hidden="true" />}<Routes><Route path="/" element={<Home />} /><Route path="/works/" element={<WorksList />} /><Route path="/works/:slug" element={<WorkDetail />} /><Route path="/about/" element={<AboutReference />} /><Route path="*" element={<Home />} /></Routes></div>;
}
