...
function Header() {
  ...
  return <header className="site-header"><div className="header-left"><Link className="brand-link" to="/">LEE JUHAN</Link><Link className="about-link" to="/about/">About <span aria-hidden="true">↗</span></Link><Link className="works-link" to="/works/">Works <span aria-hidden="true">↗</span></Link></div><div className="header-right"><a href="https://github.com/LH99Tw" target="_blank" rel="noopener noreferrer">GitHub ↗</a><a href="https://lh99tw.github.io" target="_blank" rel="noopener noreferrer">GitBlog ↗</a></div><Link className={`menu-button ${aboutOpen ? 'is-open' : ''}`} to={target} onClick={() => setAboutOpen((open) => !open)} aria-label={aboutOpen ? 'Close About page' : 'Open About page'} aria-pressed={aboutOpen}><span /><span /><span /></Link></header>;
}
...
const liquidVertexShader = `varying vec2 vUv; uniform float progress; uniform float direction; float diagonalTravel(vec2 uv){ return uv.x*0.72+(1.0-uv.y)*0.28; } void main(){ vUv=uv; vec3 p=position; float rawTravel=diagonalTravel(uv); float travel=direction>0.0?rawTravel:1.0-rawTravel; float pulse=smoothstep(0.0,0.12,progress)*smoothstep(1.0,0.88,progress); float boundary=direction>0.0?1.0-progress:progress; float bulge=exp(-pow((travel-boundary)/0.075,2.0))*pulse; p.x+=sin(uv.y*3.14159)*0.022*bulge; p.z+=0.16*bulge; gl_Position=projectionMatrix*modelViewMatrix*vec4(p,1.0); }`;
const liquidFragmentShader = `varying vec2 vUv; uniform sampler2D texture1; uniform sampler2D texture2; uniform float progress; uniform float direction; uniform float planeAspect; uniform float imageAspect1; uniform float imageAspect2; vec2 coverUv(vec2 uv,float imageAspect){ if(imageAspect>planeAspect){ uv.x=(uv.x-0.5)*(planeAspect/imageAspect)+0.5; } else { uv.y=(uv.y-0.5)*(imageAspect/planeAspect)+0.5; } return uv; } void main(){ float rawTravel=vUv.x*0.72+(1.0-vUv.y)*0.28; float travel=direction>0.0?rawTravel:1.0-rawTravel; float boundary=direction>0.0?1.0-progress:progress; float width=0.055; float sweep=direction>0.0?smoothstep(boundary-width,boundary+width,travel):1.0-smoothstep(boundary-width,boundary+width,travel); float gate=smoothstep(0.0,0.12,progress); sweep*=gate; vec4 outgoing=texture2D(texture1,coverUv(vUv,imageAspect1)); vec4 incoming=texture2D(texture2,coverUv(vUv,imageAspect2)); gl_FragColor=mix(outgoing,incoming,sweep); }`;
...
function ExternalLink({ work }) {
  ...
  return <a href={urls[work.slug]} target="_blank" rel="noopener noreferrer">Open evidence ↗</a>;
}
...
function About() {
  ...
  return <main className="light-page about-page about-reference">
    ...
    <section className="about-reference-footer"><div><span className="about-reference-kicker">CONTACT / KEEP IN TOUCH</span><h2>다섯 개의 창구로<br /><em>연결되어 있습니다.</em></h2></div><nav className="about-contact-icons" aria-label="Contact links">{contactLinks.map(({ label, href, icon }) => <a href={href} key={label} target={href.startsWith('mailto:') ? undefined : '_blank'} rel={href.startsWith('mailto:') ? undefined : 'noopener noreferrer'} aria-label={label}><img src={icon} alt="" /><span>{label}</span></a>)}</nav><Link className="about-reference-back" to="/">← Back to the reel</Link></section>
  </main>;
}
...
function AboutReference() {
  ...
  return <main className="light-page about-reference about-reference-grid">
    ...
    <footer className="about-grid-footer"><div><div className="about-grid-contact-icons">{contactLinks.map(({ label, href, icon }) => <a key={label} href={href} target={href.startsWith('mailto:') ? undefined : '_blank'} rel={href.startsWith('mailto:') ? undefined : 'noopener noreferrer'} aria-label={label}><img src={icon} alt="" /><span>{label}</span></a>)}</div></div><div className="about-grid-footer-mark" aria-hidden="true"><i /><i /><i /><i /><i /><i /></div></footer>
  </main>;
}
